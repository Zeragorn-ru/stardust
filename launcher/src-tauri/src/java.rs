...
pub fn list_download_vendors() -> Vec<JavaVendorInfo> {
    [
        ...
        (JavaVendor::Microsoft, "Microsoft Build of OpenJDK", "Microsoft"),
...
fn collect_installations(data_dir: &Path, deep: bool) -> Vec<JavaInstallation> {
    ...
    let mut push_home = |home: PathBuf, source: &str| {
        let home = match home.canonicalize().or_else(|_| Ok::<_, std::io::Error>(home.clone())) {
            Ok(p) => p,
            Err(_) => home,
        };
...
pub async fn resolve_java(
    ...
    match provider {
        ...
        JavaProvider::Temurin => ensure_downloaded_java(JavaVendor::Temurin, progress, http, data_dir).await,
        JavaProvider::Auto => {
...
pub async fn download_java(
    ...
    if is_zip {
        ...
        crate::minecraft::download_to(progress, http, &url, &archive, "Java 21 runtime", None, None)
            .await?;
...
async fn resolve_download_url(vendor: JavaVendor, http: &reqwest::Client) -> Result<String, String> {
    match vendor {
...
fn deep_search_dir(dir: &Path, depth: usize, max_depth: usize, push_home: &mut impl FnMut(PathBuf, &str)) {
    if depth > max_depth {
        return;
    }
...
fn discover_macos(push_home: &mut impl FnMut(PathBuf, &str)) {
    ...
    if let Ok(output) = Command::new("/usr/libexec/java_home").arg("-V").output() {
        ...
        for line in text.lines().skip(1) {
            if let Some(home) = line.split_whitespace().filter(|t| t.starts_with('/')).last() {
