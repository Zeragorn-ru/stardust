...
### 1. Безопасность
- [ ] Перестать записывать bearer-токен в `session.json`; хранить его только в keyring
- [ ] Убрать жёстко прошитый builtin-прокси из обновлятора и унифицировать его с настройками лаунчера
- [ ] Довести хеширование паролей до полноценного Argon2 для новых записей, а не только fallback-проверки
- [ ] Пройтись по критичным эндпоинтам и явно задокументировать rate limiting

### 2. Качество и тесты
- [ ] Добавить frontend-тесты или хотя бы smoke-проверки для `launcher` и `admin-web`
...
- [ ] Продолжить полировку UX обновлений и статусов скачивания
- [ ] Расширить настройки runtime и диагностику проблем запуска

### 4. Серверная интеграция
- [ ] Упростить развёртывание полного контура (compose/nix/scripts)
- [ ] Описать production-схему reverse proxy, TLS и доменов
- [ ] Дожать поток кастомизации/скинов до полностью документированного прод-сценария

### 5. Мод и кастомизация
- [ ] Расширить runtime-интеграцию мода beyond TAB
- [ ] Добавить серверную диагностику и админские инструменты для кастомизации
